...
/**
 * Finds the next or previous power of two relative to the given value.
 * @param value the current value
 * @param backwards if true, finds the previous power of two; otherwise, finds the next
 * @return the nearest power of two in the requested direction
 */
public static long nearestPowerOfTwo(long value, boolean backwards) {
    if (value <= 0) return 1;
    int highest = 64 - Long.numberOfLeadingZeros(value - 1);
    long currentPower = 1L << (highest - 1);

    if (backwards) {
        long previous = currentPower >> 1;
        return Math.max(previous, 1);
    } else {
        long next = currentPower << 1;
        return next;
    }
}
    ...
    public static long getSeed(Vector3 vec3i) {
        return Mth.getSeed((int)Math.floor(vec3i.x), (int)Math.floor(vec3i.y), (int)Math.floor(vec3i.z));
    }
    ...
    public static double lerp3(double d, double d2, double d3, double d4, double d5, double d6, double d7, double d8, double d9, double d10, double d11) {
     ...
     public static float inverseLerp(float a, float b, float c) {
       ...
       public static float map(float number, float oldMin, float oldMax, float newMin, float newMax) {
        return lerp(inverseLerp(number, oldMin, oldMax), newMin, newMax);
    }
